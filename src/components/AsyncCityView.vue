...
const getWeatherData = async () => {
    try{
        ...
        const weatherData = await axios.get(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude={part}&appid=${openWeatherAPIKey}&units=metric`
        );

        //Take into account timezone of the city
        ...
        weatherData.data.currentTime = 
            utc + 1000 * timezoneOffset;

        //Calculate the hourly weather offset
        weatherData.data.hourly.forEach((hour) => {
            const utc = hour.dt * 1000 + localOffset;
            hour.currentTime = utc + 1000 * timezoneOffset;
        });
